...
React 是一個用於呈現使用者介面（UI）的 JavaScript 函式庫。UI 是由像按鈕、文字和圖像這樣的小單位所構建而成。
...
> 在 HTML 中，例如，`<div>`、`<p>` 和 `<span>` 就是標籤，它們用於將文本和內容組織成可理解的格式。在
> React 中，JSX 也是一種類似標籤的語法，用於定義 UI 元素的結構。
