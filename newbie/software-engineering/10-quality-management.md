...
> 簡單來說，品質保證 (QA) 是一種系統地監控和評估用於創造產品的各種流程的過程，以確保最終輸出符合指定的品質標準。

### 3. Quality Control (QC) - 品質控制
...
## 軟體品質管理的最佳實踐 ([參考文章](https://blog.kms-solutions.asia/software-quality-management))
...
以下另外提供一些簡單例子，說明品質管理中的各個方面如何在實際情況中運作：

1. **品質計畫**：

   例如：在開發一個新的網頁應用程式時，團隊制定了品質計畫，設定了性能標準（如回應時間不超過 2 秒）和安全標準（如所有使用者數據必須經過加密）。這些標準將指導開發和測試過程。

2. **品質保證**：

   例如：在開發過程中，團隊定期進行代碼審查，確保代碼符合編碼規範和最佳實踐。這樣做可以及早發現問題，避免將錯誤引入產品。

3. **品質控制**：

   例如：在測試階段，團隊使用自動化測試工具對應用程式進行回歸測試，檢查新功能是否破壞了現有功能。測試結果會被記錄並分析，以確保產品符合預期標準。

4. **錯誤管理**：

   例如：在測試過程中發現一個顯示錯誤，測試人員會在錯誤追蹤系統中記錄該錯誤，並指定負責的開發人員進行修正。錯誤的修正進度會被追蹤，直到問題得到解決。

5. **品質改進**：

   例如：在一次產品發布後，團隊分析了客戶回饋和錯誤報告，發現多數問題與使用者界面的易用性有關。基於這些回饋，團隊決定在下一版本中改善界面設計和使用者體驗。

6. **標準和流程制定**：

   例如：開發團隊制定了一套標準化的開發流程，包括需求收集、設計、開發、測試和部署。這些流程包括明確的步驟和檢查點，確保每個階段都符合品質要求。
