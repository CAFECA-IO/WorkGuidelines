...
傳統的網頁設計方式是針對不同的裝置做出多個不同的網頁，當使用者進入網頁時，由伺服器判斷使用的設備，再將使用者導引至對應的網頁。由於網頁沒有統一，這種設計方式將造成以下幾種缺點：
  - 網頁製作成本高，且維護不易
  - 伺服器判斷錯誤或使用者直接點擊特定裝置的連結，導致顯示與裝置不符的頁面，將影響使用者體驗
  - 網址重複多次可能影響 SEO 搜尋排名
...
# 4. 其他技巧
使用框架工具(如：Boostrap)雖然可以節省許多時間，但會有大量的累贅程式碼，修改起來不容易，也會拉低網頁的 SEO 效能。而手工切版可以精簡程式，掌握較多的程式自主權，也可以達到 HTML 最佳化，缺點就是需要很長的製作時間，
